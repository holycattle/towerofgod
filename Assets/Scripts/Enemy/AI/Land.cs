...
public class Land : MonoBehaviour {
	...
	public const float ATTACK_RANGE = 0.25f;
	public const float SIGHT_RANGE = 2f;
	...
	void FixedUpdate () {
		...
		if(target == null) {
			if(Mathf.Abs(initialX - enemyObject.transform.position.x) < AUTO_MOVE_RANGE) {
				enemyObject.transform.Translate(new Vector2(Time.fixedDeltaTime * moveSpeed/2f * direction, 0));
			} else {
				initialX = enemyObject.transform.position.x;
				Flip();
			}
			...
			if(Mathf.Abs(target.position.x - enemyObject.transform.position.x) <= SIGHT_RANGE) {
